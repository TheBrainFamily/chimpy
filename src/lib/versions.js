...
function Versions(options) {
  ...
  this.getJavaVersion = () => this._spawnSync('java -version');
  ...
  this.getOsVersion = () => `${os.type()} ${os.release()}`;

  this.getNodeVersion = () => process.version;
  ...
  this.getCurrentBrowserVersion = (callback) => {
    ...
    } else if (fs.existsSync(this.chromeDriverExec)) {
      const startBrowserOptions = {
